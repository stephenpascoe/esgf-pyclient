...
def test_ceda_ats():
    ...
    resp = service.send_request(openid=openid, 
                                attributes=['urn:esg:first:name', 'urn:esg:last:name'])
...
def test_unkown_principal():
    ...
    resp = service.send_request(openid=openid, attributes=[])
...
def test_multi_attribute():
    ...
    openid = 'https://ceda.ac.uk/openid/Stephen.Pascoe'
    resp = service.send_request(openid=openid, atributes=['CMIP5 Research'])
